...
/**
 * Class WC_Datacap_Gateway
 */
class WC_Datacap_Gateway extends WC_Payment_Gateway_CC
{
    const VISA = 'visa';
    ...
    const PROD_WEBTOKEN_URL = 'https://token.dcap.com/v1/client';
    const CERT_WEBTOKEN_URL = 'https://token-cert.dcap.com/v1/client';

    const CONFIG_ROOT = 'woocommerce_datacap_settings';
     ...
     * @var int
     */
    public static $checkoutCounter = 0;


    /**
     * WC_Gateway_Datacap constructor.
     */
    public function __construct()
    {
        $this->id = WC_DATACAP_PAYMENT_METHOD_ID;
        $this->icon = "";
        ...
        $this->method_title = 'Credit Card (Datacap)';
    ...
    public function add_credit_card_fields($fields, $paymentMethod)
    {
        ...
        if ((is_checkout() || is_checkout_pay_page()) && $this->is_level_ii_enabled()) {
            $fields = array_merge($fields, array(
                'datacap-po-number' => '<p class="form-row form-row-wide">
                    <label for="' . esc_attr($this->id) . '-po-number">' . esc_html__('Purchase Order Number', WC_DATACAP_MODULE_NAME) . '</label>
                    <input id="' . esc_attr($this->id) . '-po-number" name="' . $this->id . '_po_number" class="input-text" inputmode="numeric" autocorrect="no" autocapitalize="no" spellcheck="no" placeholder="' . __('Purchase Order Number', WC_DATACAP_MODULE_NAME) . '" />
                </p>',
            ));
        }
        ...
        if (is_add_payment_method_page()) {
            $fields = array_merge($fields, array(
                'datacap-billing-zip' => '<p class="form-row form-row-first">
     ...
     * @return bool
     */
    protected function datacap_card_brand_to_wc($datacapBrand)
    {
        require_once(WC_DATACAP_PLUGIN_DIR . '/lib/api/response/class-wc-datacap-api-response-credit-transaction.php');

        $brands = array(
            WC_Datacap_API_Response_Credit_Transaction::BRAND_VISA => self::VISA,
            WC_Datacap_API_Response_Credit_Transaction::BRAND_MASTERCARD => self::MASTERCARD,
            WC_Datacap_API_Response_Credit_Transaction::BRAND_DISCOVER => self::DISCOVER,
            WC_Datacap_API_Response_Credit_Transaction::BRAND_AMERICAN_EXPRESS => self::AMERICAN_EXPRESS,
//            WC_Datacap_API_Response_Credit_Transaction::BRAND_DCLB => 'diners',
//            WC_Datacap_API_Response_Credit_Transaction::BRAND_JCB => 'jcb',
            WC_Datacap_API_Response_Credit_Transaction::BRAND_OTHER => 'other'
        );

        return (isset($brands[$datacapBrand]) ? $brands[$datacapBrand] : $brands[WC_Datacap_API_Response_Credit_Transaction::BRAND_OTHER]);
    }
     ...
     * @return bool
     */
    public function can_use_datacap_brand($brand)
    {
        return $this->can_use_brand($this->datacap_card_brand_to_wc($brand));
    }
    ...
    public function init_form_fields()
    {
        ...
        $this->form_fields = require(WC_DATACAP_PLUGIN_DIR . '/lib/admin/datacap-settings.php');
    }
    ...
    public function add_payment_method()
    {
        ...
        if (!$token || !$brand || !$expMonth || !$expYear || !$last4) {
            wc_add_notice(__('There was a problem adding this card.', 'woocommerce'), 'error');
            return null;
        }

        if (!$this->can_use_datacap_brand($brand)) {
            wc_add_notice(__('You cannot use this card type at this store (' . $brand . ')', WC_DATACAP_MODULE_NAME), 'error');
            return null;
        }
        ...
        try {
            ...
            wc_add_notice(sprintf(__('An error occurred while verifying your card details. If this problem persists, please contact support. (%s)', WC_DATACAP_MODULE_NAME), $e->getMessage()), 'error');
            return null;
        }

        $token = $this->add_card($cardToken, $this->datacap_card_brand_to_wc($brand), $last4, $expMonth, $expYear);
    ...
    {
        require_once(WC_DATACAP_PLUGIN_DIR . '/lib/api/credit/class-wc-datacap-api-credit-authonly.php');
        $request = new WC_Datacap_API_Credit_Authonly();
    ...
    public function payment_fields()
    {
        ...
        wp_enqueue_style(
            'wc-datacap-styles',
            plugins_url('assets/css/datacap.css', WC_DATACAP_ENTRY_FILE),
            null
        );
        ...
        wp_enqueue_script(
            'wc-datacap-tokenization',
            plugins_url('assets/js/frontend/checkout.js', WC_DATACAP_ENTRY_FILE),
            null
        );
    }
    ...
    public function process_payment($order_id)
    {
        ...
        if (!$response->isSuccessful()) {
            wc_add_notice(__('Payment error:', WC_DATACAP_MODULE_NAME) . sprintf(' Your transaction was unsuccessful. Please verify your payment details. (%s)', $response->getMessage()), 'error');
            return null;
        }
        ...
        if ($this->should_auth_and_capture()) {
            ...
            $order->update_status('on-hold', sprintf(__('Charge authorized. (Refno: %s). Process order to take payment, or cancel to remove the pre-authorization.', WC_DATACAP_MODULE_NAME), $response->getRefNo()));
            $order->save();
        }
        ...
        if ($this->can_save_cards() && $saveNewCard && $response->getToken()) {
            $this->cardBrand = $this->datacap_card_brand_to_wc($response->getBrand());
     ...
     * @return null|WC_Datacap_API_Response_Credit_Transaction
     */
    protected function execute_sale_request($order)
    {
        ...
        $request = new WC_Datacap_API_Credit_Sale();
    ...
    protected function setup_api_request_parameters($request, $order)
    {
        ...
        if (!$areCardFieldsFilled && !$hasToken && ($savedCardId === 'new' || !is_numeric($savedCardId))) {
            wc_add_notice(__('Payment error:', WC_DATACAP_MODULE_NAME) . ' Please verify you have filled in all of the required fields.', 'error');
            return null;
        }
        ...
        if ($savedCardId !== 'new' && is_numeric($savedCardId)) {
            ...
            $canUseCard = $this->can_use_brand($this->datacap_card_brand_to_wc($tokenBrand));
    ...
    public function process_refund($order_id, $amount = null, $reason = '')
    {
        ...
        require_once(WC_DATACAP_PLUGIN_DIR . '/lib/api/credit/class-wc-datacap-api-credit-return.php');
        $request = new WC_Datacap_API_Credit_Return();
        $request->setToken($cardToken);
        $request->setAmount($amount);

        /** @var WC_Datacap_API_Response_Credit_Transaction $response */
        $response = WC_Datacap_Gateway_Entry::getApiInstance()->send($request, array(
            'RefNo' => $transactionId
        ));

        if (!$response->isSuccessful()) {
            return new WP_Error('datacap_refund_error', sprintf('Your refund request was unsuccessful. Message: %s', $response->getMessage()));
        }

        $order->add_order_note(sprintf(__('This order has been refunded %s. RefNo: %s', WC_DATACAP_MODULE_NAME), $amount, $response->getRefNo()));
